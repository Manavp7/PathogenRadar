"""Risk endpoints: latest risk per district, historical snapshots, per-district detail."""
...
def get_risk(
    ...
    rows = _snapshot(rs, as_of) if as_of else rs.risk_latest
    return paginate(rows, limit, offset)


@router.get("/timeline")
def get_timeline(rs: RegionState = Depends(region_state)) -> dict:
    ts = rs.risk_ts
    if ts.empty:
        return {"dates": [], "series": []}
    g = ts.groupby("date")["risk_score"].agg(["mean", "max"]).reset_index().sort_values("date")
    series = [
        {"date": d.strftime("%Y-%m-%d"), "mean": round(float(m), 2), "max": round(float(mx), 2)}
...
def get_risk_for_district(district_id: str, rs: RegionState = Depends(region_state)) -> dict:
    latest = next((r for r in rs.risk_latest if r["district_id"] == district_id), None)
    if latest is None:
    ...
    if not ts.empty:
        ...
        timeseries = [
            {"date": d.strftime("%Y-%m-%d"), "risk_score": float(r), "level": lvl}
...
def _snapshot(rs: RegionState, as_of: str) -> list[dict]:
    ts = rs.risk_ts
    if ts.empty:
        return []
    try:
